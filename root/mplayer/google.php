...
* @version: $Id: google.php, v 0.0.7 2009/10/16 16:10:09Z ameisez Exp $
...
$google_playlist		= "videofeed?type=top100new";

$feed_link	= request_var('rssfeed', $google_playlist);
$feed_cache	= "_mplayer_rss_google" . str_replace(array('/', '&', '?', '='), array('_', '_', '_', '_'), $feed_link);
$feed_link	= "http://video.google.com/" . $feed_link . "&num=50&output=rss&sitesearch=google.com";
$contents	= bb3_rss_load($feed_link, $feed_cache);
$result		= xml2array($contents);
$feed_ary	= array();
$feed_title	= $user->lang['BB3_GOOGLE_PLAYLIST'];
if (sizeof($result))
{
	...
	for ($i = 0, $size = sizeof($items); $i < $size; $i++)
	{
		...
		if (!isset($items[$i]['media:group']['media:content']['attr']['duration']))
		{
